...
`prismo run --seed {u,lateral,vertical}` picks the initial topology on the design nodes at `|θ| = 0.3` (`pipeline.seed_design_field`); `u` is the CLI default, being the best of the three on the container mesh. `lateral` is the **seeded junction** above. `vertical`: in the rib, n-type below the rib's mid-height and p-type above it, with a p column along the rib's right wall (a quarter of the rib width) joining the p top to the right slab; the slab itself stays lateral. `u`: n-type wraps under (the lower third of the rib) and beside (the left wall and left slab) a p core, which with the right wall and right slab is p-type. Every seed keeps n-type on the left slab edge (anode) and p-type on the right (cathode at −5 V), so both carrier populations reach a contact and the seed is reverse-biased. The MMA optimum is local, so the seed picks the basin: under matched settings on the 0.05 µm mesh the `u` seed reaches VπLπ = 0.62 V·cm against 1.10 (`lateral`) and 1.04 (`vertical`), and on every mesh its optimum closes the U into a ring — a p core at the doping ceiling enclosed by n on all sides, the junction sitting on the mode centre. The `lateral` seed's own optimum is an L-shaped junction, the 2D cross-section of the literature's L-/U-shaped junctions.
